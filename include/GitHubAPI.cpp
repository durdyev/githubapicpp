...
std::vector<githubapicpp::User> githubapicpp::GitHubAPI::getUsers(int since) {
    ...
    if (doc.IsArray()) {
        for (auto &el : doc.GetArray()) {
            usersVector.push_back(convertJSONToUser(el.GetObject()));
        }
    }
    return usersVector;
}

githubapicpp::User githubapicpp::GitHubAPI::convertJSONToUser(rapidjson::Value el) {
    User * user = new User();
    if (el.HasMember("id"))
